...
class RoomChatDataSourceImpl(
    ...
    override suspend fun insertRoomChat(roomChat: RoomChat): Unit = withContext(Dispatchers.IO) {
    ...
    override suspend fun findAllRoomChatsById(userId: String): List<RoomChat> = withContext(Dispatchers.IO) {
        db.getCollection<RoomChat>(ROOM_CHAT).find(or(RoomChat::menteeId eq userId, RoomChat::mentorId eq userId)).toList()
    }
